...
- **что просрочено** — протух ли снимок текущего состояния, есть ли ожидания с прошедшей датой, давно ли разбирали журнал и прогоняли контрольные вопросы, есть ли незакоммиченное, не выросла ли база настолько, что структуру пора пересматривать, не отстала ли редакция контракта от установленной;
...
- линтер на две тысячи строк, ни разу не проверенный на живом репозитории, **удалён**. Заменён четырьмя проверками, каждая из которых либо находит настоящую поломку, либо молчит.
...
plugins/kb-architect/skills/kb-architect/
  ...
  assets/templates/     18 заготовок: правила проекта, вход, канал правок,
                        журнал, контрольные вопросы, хендовер, отчёт о дефекте
