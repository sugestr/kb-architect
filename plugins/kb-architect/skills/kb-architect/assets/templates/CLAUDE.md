...
5. Перед записью в область: `kb_debts.py <корень> --area <путь области>`; относящиеся
   долги знания закрываются в этой работе или получают PENDING с адресом.
...
kb_standard_version: <минимальная совместимая версия проекта, сейчас 7.2.0>
...
- Производный файл получает `generated_from` и правится в источнике; адрес переживает
  сессию (tracked путь, маршрут/query или именованный внешний канон), не scratchpad.
- Новый файл знания получает маршрут или ссылку от точки входа области в тот же
  коммит; `kb_check` показывает знание без дороги от индекса.
- Работа оставляет знание в том же коммите: изменённый модуль кода — его описание,
  внесённый входящий — провенанс `message_id` или строка реестра разбора, внешнее
  действие — квитанция и current. `kb_debts.py` показывает, что не дошло.
