...
Это один физический канон правил: `CLAUDE.md` и `AGENTS.md` связаны
symlink на одни байты. Текущее состояние ведётся только в [NOW.md](NOW.md).
...
1. Один раз на новую task прочитай установленный `kb-architect/SKILL.md` по пути
   из каталога скиллов, до предметной интерпретации; отсутствие назови явно.
...
kb_standard_version: <минимальная совместимая версия проекта, сейчас 7.1.0>
...
<Если сервисный контур принят, new task после первого безопасного результата выполняет delivery
+ `kb_apply.py`; дельта принимается до затронутых зависимых действий. `NEEDS_APPLICATION`/`APPLICATION_UNPROVEN` остаётся до короткой
...
- Текущее состояние проекта: `NOW.md`; индекс current ведёт только туда.
