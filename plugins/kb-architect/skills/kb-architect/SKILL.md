...
Не читай весь пакет: принятый проект работает по своим boot/current и роли.
...
- Существенный project-derived вывод требует evidence по `retrieval.md`.
  Принятый query/ledger и прямые поля источника lexical receipt не требуют.
  Пустой поиск и механический PASS не доказывают полноту/истину.
...
- Новый минимальный уровень проекта принимается один раз (`migration.md`); patch
  миграцию не повторяет, установка marker не повышает. Update — `service-layer.md`.
...
Локальный источник обрабатывай сразу, обязательные проверки — параллельно; открывай
только относящийся reference. Для ответа без новой
`SOURCE / FACT / INTERPRETATION / DECISION / OPEN` служебный tail не нужен.
Изменение — в каноне и зависимых представлениях, один пакет относящихся проверок,
точечный коммит блока при полномочиях. Работа оставляет
знание в том же коммите: модуль — описание, входящий — провенанс `message_id`,
внешнее действие — квитанцию и current. Незавершённое — `DURABLE_TAIL=PENDING` с
адресом продолжения при коммите. Объявленный вход сохраняется целиком; часть не
выдаётся за партию. Перед записью в область — `kb_debts.py <root> --area
<путь>`; относящееся закрой или назови PENDING.

Ошибку, противоречие и неполноту показывай в границах вывода. Повтор дефекта или
лишней работы — сигнал к улучшению; цену (`kb_cost.py`), пользу и ошибки оценивай вместе.
