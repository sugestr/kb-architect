...
def main():
    ...
    if skill_version_now:
        ...
        # Источник: не вышла ли редакция новее установленной. Локально,
        # без сети — если установка сделана симлинком в репозиторий.
        # Обновление не выполняется автоматически: поставить новую версию
        # и применить её к базе — разные действия, и второе делается руками.
        _, behind = kb_paths.published_version()
        ...
        if behind and auto and auto.strip().lower().startswith(("авто", "auto", "да", "yes")):
            ...
            if was:
                due.append(f"скилл обновлён автоматически: {was} → {now_v}. "
                           ...
                           f"(references/rationale.md, строки между {was} и {now_v}), обнови "
            ...
            due.append(f"источник скилла ушёл вперёд на {behind} коммит(ов) — вышла редакция "
                       ...
                       f"прочитай в references/rationale.md строки выпусков между своей "
                       f"и новой, возьми применимое, обнови kb_standard_version и запиши "
                       f"в «Соответствие», что взял и от чего отказался. Установка без "
                       f"применения — это новый код при старых соглашениях")
        elif behind == 0:
            ok.append("источник скилла: новее ничего не опубликовано")
        else:
            ok.append("источник скилла отсюда не виден (установка файлом, а не из "
                      "репозитория) — узнать о новой редакции можно только вручную")
