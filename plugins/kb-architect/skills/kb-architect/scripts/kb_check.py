...
что именно проверено, и промах поиска входа сам становится находкой.
  ...
  1. Локальная цель ссылки не найдена. Проверяется путь, включая file:line,
     а не сохранность знания в других источниках.
  ...
  5. Ветки с неинтегрированными изменениями. Проверка называет ветки,
     даты и отличающиеся версии файлов для сверки. Семантическая полнота
     базы этим не проверяется.

  6. Адресация инбокса: локальная копия исходящего либо адресат, не совпавший
     с identity проекта. Доставка и прочтение этой проверкой не устанавливаются.
...
def main():
    ...
    for path in files_all:
        ...
        for target in LINK.findall(body):
            ...
            if not os.path.exists(near) and not os.path.exists(from_root):
                # A positive line suffix addresses a file, not a filename.
                # Existing literal colon filenames retain the check above;
                # additional colons (URI/drive/column forms) are not reinterpreted.
                locator = LINE_LOCATOR.fullmatch(clean)
                if locator:
                    located_near = os.path.join(os.path.dirname(path), locator.group(1))
                    located_root = os.path.join(root, locator.group(1).lstrip("/"))
    ...
    if nedostavleno:
        ...
        print("  Найдена локальная копия с from_project этого проекта.")
        print("  Она не подтверждает этап доставки или прочтение адресатом.")
        print("  Проверь инбокс адресата, его коммит или квитанцию с тем же message_id.\n")

    if chuzhie:
        found += len(chuzhie)
        print(f"АДРЕСАТ ИНБОКСА НЕ СОПОСТАВЛЕН С ПРОЕКТОМ — {skl(len(chuzhie), 'файл', 'файла', 'файлов')}:")
        for rel, komu in chuzhie[:10]:
            print(f"  {rel} → адресовано: {komu}")
        print("  Указанный адресат не совпал с обнаруженной identity проекта.")
        print("  Проверь recipient, project_aliases и квитанцию с тем же message_id.")
        print("  Доставка и прочтение этой проверкой не установлены.\n")
    ...
    if poterya:
        found += len(poterya)
        print(f"РАЗЛИЧАЮТСЯ ВЕРСИИ В ВЕТКАХ — {skl(len(poterya), 'ветка', 'ветки', 'веток')} "
              f"для сверки с HEAD:")
        for v in poterya:
            ...
            print(f"  {v.name} — последний коммит {v.last}, "
                  f"{skl(v.lost, 'версия файла', 'версии файлов', 'версий файлов')} для сверки "
                  f"(по путям {len(v.outside_path)}){hvost}")
            for p in v.outside_name[:5]:
                print(f"      {p}")
            if v.lost > 5:
                print(f"      … и ещё {v.lost - 5}")
        print("  Различие не доказывает, что работа не переносилась: возможны cherry-pick,")
        print("  последующая правка или отмена. Сверь историю и актуальность версий;")
        print("  только затем реши, нужно ли переносить изменения.")
        print("  При изменениях с обеих сторон проверь конфликты и актуальность версий.\n")
