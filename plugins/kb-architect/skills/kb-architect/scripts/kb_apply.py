...
Код 1 означает `NEEDS_APPLICATION`: проект отстаёт и должен закрыть перечисленную
дельту. Это не crash, а машинно заметное незавершённое применение. Код 0 возможен
только когда объявленная редакция проекта уже не ниже установленной.
