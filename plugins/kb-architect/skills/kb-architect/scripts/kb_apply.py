...
# обязанности проекта. Всё остальное — правки инструментов и текста: они
# приезжают сами вместе с установкой и от проекта ничего не требуют.
# До 4.17 скрипт вываливал все выпуски подряд, и владелец должен был читать
# семнадцать статей, чтобы найти три дела. Это не механизм, а домашнее задание.
...
def main():
    ...
    for v, text in rows:
        m = ACT.search(text)
        if m:
            dela.append((v, " ".join(m.group(1).split())))

    print(f"Проект на {proj}, установлен скилл {inst}. Между ними выпусков: {len(rows)}.\n")

    if not dela:
        print("ДЕЛ НЕТ. Все правки в этом промежутке — внутри инструментов и текста")
        print("справочника: они уже работают, потому что скилл установлен. От базы")
        print("ничего не требуется.\n")
    else:
    ...
    print("  1. сделать перечисленное выше — это и есть «применить редакцию»;")
    print("  2. прогнать kb_check.py и kb_due.py: они написаны новее вашей")
    print("     редакции и найдут ошибки, которые база могла накопить под старой;")
    print("  3. поправить найденное, каждую правку — записью в канал с адресом;")
    print("  4. обновить kb_standard_version на " + str(inst) + " и записать")
